...
public class CheckVoucher extends _JInternalFrame implements _GUI, ActionListener, MouseListener {

	/*
	 * AS 2021-11-08
	 * 
	 * 1. MODIFY PREVIEW BUTTON - FOR TREASURY AND LLD, MAKE A MC VERSION OF
	 * PREVIEWING CHECKS NO DCRF 2021-11-08 2. ADDITIONAL BANK ACCT FOR VDC
	 * COMMISSION 2021-11-23
	 * 
	 */
	...
	static String title = "Disbursement Voucher (DV)";
	...
	public CheckVoucher(String title) {
		super(title);
		// TODO Auto-generated constructor stub
	}

	public CheckVoucher(String title, boolean resizable, boolean closable, boolean maximizable, boolean iconifiable) {
				...
				lookupDV_no.addLookupListener(new LookupListener() {
					public void lookupPerformed(LookupEvent event) {
						...
						if (data != null) {
							...
							displayDV_account_entries(modelDV_acct_entries, rowHeaderDV_acct_entries, modelDV_accttotal,
									cv_no);
				...
				lookupBankAcctNo.addLookupListener(new LookupListener() {
					public void lookupPerformed(LookupEvent event) {
						...
						if (data != null) {
							...
							System.out.printf("Display value of Acct ID: %s%n", acct_id);
...

			{
...

				{
					...
					}
					{
						...
						scrollDV_acct_entries.setCorner(ScrollPaneConstants.UPPER_LEFT_CORNER,
			...
			}
			{
...

				{
					...
					pnlDV_pv.add(scrollDV_pv, BorderLayout.CENTER);
					{
						...
						tblDV_pv.hideColumns("Check No", "Check Date");
	...
	// display tables
	public static void displayDV_account_entries(DefaultTableModel modelMain, JList rowHeader,
	...
	public static void displayDV_pv(DefaultTableModel modelMain, JList rowHeader, DefaultTableModel modelTotal,
			String req_no) {// used
	...
	public static void enableButtons(Boolean a, Boolean b, Boolean c, Boolean d, Boolean e, Boolean f, Boolean g,
	...
	public void actionPerformed(ActionEvent e) {// used
		...
		if (e.getActionCommand().equals("Add") && FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == true) {
			addnew();
		} else if (e.getActionCommand().equals("Add")
				&& FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == false) {
			JOptionPane.showMessageDialog(getContentPane(),
					"Sorry, you are not authorized to create a new disbursement.", "Information",
					JOptionPane.INFORMATION_MESSAGE);
		}

		if (e.getActionCommand().equals("Edit") && FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == true) {
			edit();
		} else if (e.getActionCommand().equals("Edit")
				&& FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == false) {
			JOptionPane.showMessageDialog(getContentPane(), "Sorry, you are not authorized to edit disbursement.",
					"Information", JOptionPane.INFORMATION_MESSAGE);
		}

		if (e.getActionCommand().equals("Save") && FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == true) {
			save();
		} else if (e.getActionCommand().equals("Save")
				&& FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == false) {
			JOptionPane.showMessageDialog(getContentPane(),
					"Sorry, you are not authorized to create a new disbursement.", "Warning",
					JOptionPane.WARNING_MESSAGE);
		}

		if (e.getActionCommand().equals("Tag") && FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == true) {
			tag("F", "Tag");
		} else if (e.getActionCommand().equals("Tag")
				&& FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == false) {
			JOptionPane.showMessageDialog(getContentPane(), "Sorry, you are not authorized to tag disbursement.",
					"Warning", JOptionPane.WARNING_MESSAGE);
		}

		if (e.getActionCommand().equals("Untag") && FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == true) {
			tag("A", "Untag");
		} else if (e.getActionCommand().equals("Untag")
				&& FncAcounting.EmpCanAddNew(UserInfo.EmployeeCode, "3") == false) {
			JOptionPane.showMessageDialog(getContentPane(), "Sorry, you are not authorized to untag disbursement.",
					"Warning", JOptionPane.WARNING_MESSAGE);
		}

		if (e.getActionCommand().equals("Post") && FncAcounting.EmpCanApprove(UserInfo.EmployeeCode, "3") == true) {
			post();
		} else if (e.getActionCommand().equals("Post")
				&& FncAcounting.EmpCanApprove(UserInfo.EmployeeCode, "3") == false) {
			JOptionPane.showMessageDialog(getContentPane(), "Sorry, you are not authorized to post disbursement.",
					"Warning", JOptionPane.WARNING_MESSAGE);
		}

		if (e.getActionCommand().equals("Delete") && FncAcounting.EmpCanDelete(UserInfo.EmployeeCode, "3") == true) {
			delete();
		} else if (e.getActionCommand().equals("Delete")
				&& FncAcounting.EmpCanDelete(UserInfo.EmployeeCode, "3") == false) {
			JOptionPane.showMessageDialog(getContentPane(), "Sorry, you are not authorized to delete disbursement.",
					"Warning", JOptionPane.WARNING_MESSAGE);
		}

		if (e.getActionCommand().equals("Preview") && FncAcounting.EmpCanPreview(UserInfo.EmployeeCode, "3") == true) {
			preview_CV();
			if (lookupPaymentType.getText().trim().equals("B")) {
				preview_check();
			}
		} else if (e.getActionCommand().equals("Preview")
				&& FncAcounting.EmpCanPreview(UserInfo.EmployeeCode, "3") == false) {
			JOptionPane.showMessageDialog(getContentPane(), "Sorry, you are not authorized to issue check.", "Warning",
					JOptionPane.WARNING_MESSAGE);
		}

		/*
		 * if (e.getActionCommand().equals("Preview") &&
		 * FncAcounting.EmpCanPreview(UserInfo.EmployeeCode, "3")==true ) {
		 * preview_CV(); } else if (e.getActionCommand().equals("Preview") &&
		 * lookupPaymentType.getText().trim().equals("B") &&
		 * FncAcounting.EmpCanPreview(UserInfo.EmployeeCode, "3")==false ) {
		 * JOptionPane.showMessageDialog(getContentPane()
		 * ,"Sorry, you are not authorized to preview/print CV.","Warning",JOptionPane.
		 * WARNING_MESSAGE); }
		 */
		...
		public void mousePressed(MouseEvent ev) {
			if (ev.isPopupTrigger()) {
				...
				if (txtStatus.getText().equals("POSTED")
					...
					mniediteDatePaid.setEnabled(true);
				} else if (!txtStatus.getText().equals("POSTED")
						|| FncAcounting.EmpCanApprove(UserInfo.EmployeeCode, "3") == false) {
					mniediteDatePaid.setEnabled(false);
				}
	...
	private void tag(String status, String to_do) {
		...
		if (JOptionPane.showConfirmDialog(getContentPane(), to_do + " DV entries?", "Confirmation",
			...
			JOptionPane.showMessageDialog(null, "Disbursement Voucher (DV) " + to_do + "gged.", "Information",
					JOptionPane.INFORMATION_MESSAGE);
			if (to_do.equals("Tag")) {
				enableButtons(false, false, true, true, true, false, false, true, true, true);
			} else if (to_do.equals("Untag")) {
				enableButtons(false, true, true, true, true, false, true, false, false, true);
			}
		}
	}
	...
	private void delete() {
		...
		if (JOptionPane.showConfirmDialog(getContentPane(), "Are you sure you want to delete this DV entries?",
			...
			if (JOptionPane.showConfirmDialog(getContentPane(), "Delete DV", "Confirmation",
				...
				tagCV(dv, db, "D");
				tagPV(dv, db);
				deleteTagMCDetails(lookupCompany.getValue(), dv); //ADDED BY LESTER 2023-09-29 to delete tagged mc for pv once CV was deleted
				insertAudit_trail("Delete-Check Voucher and Remove-CV in PV", String.format("Delete cv - %s", dv), db);
				db.commit();
				enable_fields(false);
				tabCenter.setSelectedIndex(0);
				JOptionPane.showMessageDialog(null, "Disbursement Voucher (DV) deleted", "Information",
						JOptionPane.INFORMATION_MESSAGE);
				enableButtons(false, false, false, true, true, false, false, false, false, false);
	...
	private void post() {
		...
		if (isYearMonthOpen(year, month) == true) {// ---added by jed no dcrf to check if month is open---//
			...
			if (JOptionPane.showConfirmDialog(getContentPane(), "Post DV entries?", "Confirmation",
				...
				enable_fields(false);
				tabCenter.setSelectedIndex(0);
				JOptionPane.showMessageDialog(null, "Disbursement Voucher (DV) successfully posted.", "Information",
						JOptionPane.INFORMATION_MESSAGE);
			}
		...
		if (isYearMonthOpen(year, month) == true) {
			...
			if (checkCompleteDetails() == false) {
				...
				if (checkNetAmount() == false) {
					if (JOptionPane.showConfirmDialog(getContentPane(), "CV amount is equals to zero, proceed anyway?",
							"Confirmation", JOptionPane.YES_NO_CANCEL_OPTION,
							JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {
						execute_save();
					}
	...
	private Boolean checkAcctBalanceIfEqual() {// used
		...
		Double debit = Double.parseDouble(modelDV_accttotal.getValueAt(0, 2).toString());
		Double credit = Double.parseDouble(modelDV_accttotal.getValueAt(0, 3).toString());
	...
	private void preview_check() {
		...
		String check_payee = "";
		if (sql_getDefaultBroker(payee).equals("")) {
			check_payee = payee_name.trim().toUpperCase();
		} else {
	...
	private void preview_CV() {
		 ...
		 * 
		 * System.out.println("DUMAAN DITO PO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
		 * 
		 * FncReport.generateReport("/Reports/rptCV_preview_v2.jasper", reportTitle,
		 * company, mapParameters);
		 * 
		 * }
		 */
		...
		/*
		 * ADDED BY JED 2021-11-08 : MODIFY THE REPORT FOR LLD AND TREASURY FOR MC
		 * CHECKS
		 */
		if (emp_code.equals("901097") || emp_code.equals("901223") || emp_code.equals("901076")
				|| emp_code.equals("901175") || emp_code.equals("901267") || emp_code.equals("900965") || emp_code.equals("901305") || UserInfo.ADMIN) {/* for treasury */

			Object[] options = { "Original", "MC Version" };

			if (JOptionPane.showOptionDialog(getContentPane(), "Select one to preview.", "Confirmation",
					JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options,
					options[0]) == JOptionPane.YES_OPTION) {// adding of attachments

				FncReport.generateReport("/Reports/rptCV_preview.jasper", reportTitle, company, mapParameters);
			} else {
				FncReport.generateReport("/Reports/rptCV_preview_mc_v3.jasper", reportTitle, company, mapParameters);
			}
		}

		else if (checkEmployee(emp_code).equals(null)
				|| checkEmployee(emp_code).equals("")) {/* for non LLD employees */
			FncReport.generateReport("/Reports/rptCV_preview.jasper", reportTitle, company, mapParameters);
			// FncReport.generateReport("/Reports/rptCV_preview_mc_v2.jasper", reportTitle,
			// company, mapParameters);
		} else {/* for LLD employees */
			FncReport.generateReport("/Reports/rptCV_preview_mc_v3.jasper", reportTitle, company, mapParameters);
		}
	}
		...
		if (JOptionPane.showConfirmDialog(getContentPane(), "Are all entries correct?", "Confirmation",
			...
			if (to_do.equals("addnew")) {
				if (checkAcctBalanceIfEqual() == true) {// **ADDED BY JED 2019-04-22 : TO CHECK TOTAL IF BALANCE OR NOT
					...
					enable_fields(false);
					tabCenter.setSelectedIndex(0);
					JOptionPane.showMessageDialog(null, "New Disbursement Voucher (DV) saved.", "Information",
							JOptionPane.INFORMATION_MESSAGE);
					lblDV_no.setEnabled(true);
					lookupDV_no.setEnabled(true);
					mnidelete.setEnabled(false);
				...
				String pay_type = lookupPaymentType.getText();
				if (pay_type.equals("B")) {
					if(isCheckExisting(dv, lookupCompany.getValue())) {
				...
				enable_fields(false);
				tabCenter.setSelectedIndex(0);
				JOptionPane.showMessageDialog(null, "Disbursement Voucher (DV) updated.", "Information",
						JOptionPane.INFORMATION_MESSAGE);
				lblDV_no.setEnabled(true);
				lookupDV_no.setEnabled(true);
				mnidelete.setEnabled(false);
				...
				lblDatePaid.setEnabled(false);
				dtePaid.setEnabled(false);
				JOptionPane.showMessageDialog(null, "Date paid updated.", "Information",
						JOptionPane.INFORMATION_MESSAGE);
	...
	public static String getDV_no(String co_id) {// used
		return "select \r\n" + "distinct on (a.cv_no,a.co_id) " +
				...
				"from\r\n"
				+ "(select * from rf_cv_header where coalesce(remarks, '') !~* 'Released through AUB E-payment Facility' and co_id = '"
				+ co_id + "' " + "		and (case when '" + isCurrentYear
	...
	public String getPV_list() {// used
		...
		// **ADDED BY JED 2019-09-23 DCRF No. 1214 : PARA MAHUGOT UNG IBANG PV FOR THESE
		// TWO AUTOMATED CHECK PAYEE**//
		else if (payee.equals("1212121212") || payee.equals("9065461996") || payee.equals("8554349643")
				|| payee.equals("1072088811")) {
			sql = sql
					+ "and entity_id1 in ('2258478612', '1212121212', '9065461996', '8554349643', '1072088811') and pay_type_id = '"
					+ lookupPaymentType.getText() + "' ";
		}

		else if (payee.equals("9677164051")) {
			sql = sql + "and entity_id1 = '4372300431' and pay_type_id = '" + lookupPaymentType.getText() + "' ";
		}

		else {
			sql = sql + "and entity_id1 = '" + payee + "' and pay_type_id = '" + lookupPaymentType.getText() + "' ";
		}
	...
	public String sql_getAcctDesc() {// used
		...
		String SQL = "select trim(acct_name) from mf_boi_chart_of_accounts where acct_id = '" + acct_id + "' ";
	...
	public String sql_getLastCheckNo(String rec_id) {// used
		...
		String SQL = "select trim(to_check_no) " + "from rf_check_book where rec_id = " + rec_id
				+ " and trim(user_id) = '" + UserInfo.EmployeeCode + "' " + "and status_id = 'A'   \r\n";
	...
	public String sql_getLastCheckNoUsed(String rec_id) {// used
		...
		String SQL = "select trim(coalesce(last_no_used,'')) " + "from rf_check_book where rec_id = " + rec_id
				+ " and trim(user_id) = '" + UserInfo.EmployeeCode + "' " + "and status_id = 'A'   \r\n";
	...
	public static Object[] getDVdetails(String req_no) {// used
		...
		String strSQL = "---display DV header\r\n" + "select \r\n" +
				...
				+ "left join (select * from rf_check where status_id = 'A') d on a.cv_no = d.cv_no and a.co_id = d.co_id \r\n"
...
//			;
	...
	private String sql_getNextCVno(String co_id) {// used
		...
		if (db.isNotNull()) {
			if ((String) db.getResult()[0][0] == null || db.getResult()[0][0].equals("null")) {
				...
				next_cv_no = (String) db.getResult()[0][0];
			}
		} else {
			next_cv_no = "000000001";
		}
	...
	public String sql_getPV_payee() {// used
		...
		String SQL = "select distinct on (a.cv_no)\r\n" + "\r\n" + "trim(b.entity_name) \r\n" + "\r\n"
				+ "from rf_pv_header a \r\n" + "left join rf_entity b on a.entity_id1 = b.entity_id \r\n" + "\r\n"
	...
	private String sql_getDefaultBroker(String agent) {
		...
		if (db.isNotNull()) {
			if ((String) db.getResult()[0][0] == null || db.getResult()[0][0].equals("null")) {
				...
				def_broker = (String) db.getResult()[0][0];
			}
	...
	private Boolean checkCompleteDetails() {// used
		...
		String pay_type = lookupPaymentType.getText();
		if (pay_type.equals("B")) {
			if (lookupPaymentType.getText().trim().equals("") || lookupBankAcctNo.getText().trim().equals("")) {
				x = false;
			} else {
				x = true;
			}
		} else if (pay_type.equals("A")) {
			if (lookupPaymentType.getText().trim().equals("")) {
				x = false;
			} else {
				x = true;
			}
	...
	public Boolean isPVcreated() {
		...
		String SQL = "select trim(status_id) from rf_pv_header " + "where pv_no = '" + drf_no
	...
	public static void totalDV_pv(DefaultTableModel modelMain, DefaultTableModel modelTotal) {// used
		...
		for (int x = 0; x < modelMain.getRowCount(); x++) {
			...
			try {
				pv_total = pv_total.add(((BigDecimal) modelMain.getValueAt(x, 2)));
			} catch (NullPointerException e) {
				pv_total = pv_total.add(new BigDecimal(0.00));
			}
		}
	...
	private void clickTableColumn() {// used
		...
		if (column == x[column] && modelDV_pv.isEditable() && sql[column] != "") {
			...
			if (data != null && column == 0) {
				FncSystem.out("Get PV List", getPV_list());
	...
	public void insertCV_header(String rec_no, pgUpdate db) {// ok
		...
		String sqlDetail = "INSERT into rf_cv_header values (" + "'" + co_id + "',  \n" + // 1 co_id
				"'" + co_id + "',  \n" + // 2 busunit_id
				"'" + rec_no + "',  \n" + // 3 cv_no
	...
	public void insertAP_check(String rec_no, pgUpdate db) {// ok
		...
		String sqlDetail = "INSERT into rf_check values (" + "'" + co_id + "',  \n" + // 1 co_id
				"'" + co_id + "',  \n" + // 2 busunit_id
				"'" + rec_no + "',  \n" + // 3 cv_no
				"'" + check_no + "',  \n" + // 4 check_no
	...
	public void updatePV_header(String rec_no, pgUpdate db) {// ok
		...
		while (x < rw) {
			...
			String sqlDetail = "update rf_pv_header set cv_no = '" + rec_no + "' where trim(pv_no) = '" + pv_no
					+ "' and co_id = '" + co_id + "'   ";
	...
	public void updateCheck_book(pgUpdate db) {// ok
		...
		String sqlDetail = "update rf_check_book set last_no_used = '" + txtCheckNo.getText().trim() + "' "
				+ "where trim(bank_acct_id) = '" + lookupBankAcctNo.getText().trim() + "' and trim(user_id) = '"
				+ UserInfo.EmployeeCode + "' " + "and status_id = 'A' ";
	...
	public void updateCV_detail(String rec_no, pgUpdate db) {// ok
		...
		String sqlDetail = "update rf_cv_detail set status_id = 'I' where trim(cv_no) = '" + rec_no + "' and co_id = '"
				+ co_id + "'   ";
	...
	public void updateCV_headerDatePaid(String cv_no, pgUpdate db) {// ok
		...
		String bankacct = txtBankAcct.getText();
		String user = UserInfo.EmployeeCode;

		// **ADDED BY JED 2020-08-10: EDITING OF DATE PAID COMMISSION
		// **EDITED BY JED 2021-11-23 : ADDITIONAL BANK ACCT FOR VDC COMMISSION
		if (bankacct.equals("001-01-009449-3") || bankacct.equals("001-01-002813-9")) {
			if (user.equals("901115") || user.equals("901272")) {
				System.out.println("Dumaan dito COM-CARMONA");
				String sqlDetail = "update rf_cv_header set " + "proc_id = 7,  " + "date_paid = '"
						+ dateFormat.format(dtePaid.getDate()) + "',  " + "edited_by = '" + user + "',"
						+ "date_edited = now()  \n" + // 4
				...
				System.out.println("Dumaan dito COM-SUMMIT");
				String sqlDetail = "update rf_cv_header set " + "proc_id = 6,  " + "date_paid = '"
						+ dateFormat.format(dtePaid.getDate()) + "',  " + "edited_by = '" + user + "',"
						+ "date_edited = now()  \n" + // 4
						"where cv_no = '" + cv_no + "' and co_id = '" + co_id + "'   ";
			...
			String sqlDetail = "update rf_cv_header set " + "proc_id = 5,  " + "date_paid = '"
					+ dateFormat.format(dtePaid.getDate()) + "',  " + "edited_by = '" + user + "',"
					+ "date_edited = now()  \n" + // 4
					"where cv_no = '" + cv_no + "' and co_id = '" + co_id + "'   ";
		...
		/*
		 * String sqlDetail = "update rf_cv_header set " + "proc_id = 5,  " +
	...
	public void updateCV_header(String cv_no, pgUpdate db) {// ok
		...
		String sqlDetail = "update rf_cv_header set " + "co_id = '" + co_id + "',  \n" + // 1
				"busunit_id = '" + co_id + "',  \n" + // 2
				"cv_no = '" + cv_no + "',  \n" + // 3
	...
	public void updateAP_check(String rec_no, pgUpdate db) {// ok
		...
		String sqlDetail = "update rf_check set check_no = '" + txtCheckNo.getText().trim() + "',  \n"
				+ "bank_acct_id = '" + lookupBankAcctNo.getText().trim() + "', \n" + "chk_amt = "
				+ Double.valueOf(txtCheckAmt.getText().trim().replace(",", "")).doubleValue() + ", \n" + "date_due = '"
	...
	public void tagPV(String dv, pgUpdate db) {// ok
		...
		String sqlDetail = "update rf_pv_header set " + "cv_no = '', edited_by = '" + UserInfo.EmployeeCode
				+ "', date_edited = now() " + "where trim(cv_no) = '" + dv + "' and co_id = '" + co_id + "'   ";
	...
	public void postCV(String rec_no, pgUpdate db) {// ok
		...
		String sqlDetail = "update rf_cv_header " + "set status_id = 'P', " + "posted_by = '" + UserInfo.EmployeeCode
				+ "' , \n" + "date_posted = now(), \n" + "proc_id = 3 \n" + "where trim(cv_no) = '" + rec_no
				+ "' and co_id = '" + co_id + "'   ";
