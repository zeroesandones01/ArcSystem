...
public class LoanReleased extends _JInternalFrame implements ActionListener, _GUI, KeyListener {

	private static final long serialVersionUID = -6517018477299344044L;

	static String title = "Loan Released";
	...
	private GridLayout grid = new GridLayout(7, 1, 3, 3);
	...
	public LoanReleased(String entity_id, String proj_id, String pbl_id, Integer seq_no) {
		super(title, true, true, true, true);
		initGUI();
	...
	public LoanReleased(String title) {
		super(title);
		initGUI();
	}

	public LoanReleased(String title, boolean resizable, boolean closable, boolean maximizable, boolean iconifiable) {
	...
	public void initGUI() {
		...
		this.setLayout(new BorderLayout());
		{
			...
			pnlMain.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 3));
			{
				...
				pnlNorth.setPreferredSize(new Dimension(790, 400));
				{
				...
				}
				{
					...
					pnlNorth.add(pnlCenter, BorderLayout.CENTER);
					{
						...
						//pnlCol1.setBorder(components.JTBorderFactory.createTitleBorder("Receipt / Check Details"));
						...
						}
						{
							...
							}
							{
								...
								txtNetProceeds.setFont(FncLookAndFeel.systemFont_Bold.deriveFont(26f));
					...
					}
					{
						...
						pnlCol2.setBorder(JTBorderFactory.createTitleBorder("Recapitulation", FncLookAndFeel.systemFont_Bold.deriveFont(10f)));
						{
							...
							{
								JLabel lblLoanableAmount = new JLabel("Loanable Amount");
							...
							{
								JLabel lblLoanableAmount = new JLabel("Upgraded Contribution");
								//pnlLabel.add(lblLoanableAmount);
								lblLoanableAmount.setFont(font11);
							}
								...
								{
									{
										...
										txtPctgRetentionFee.getDocument().addDocumentListener(new DocumentListener() {
											...
											private void ComputeRetention() {
												...
												BigDecimal dbLoanAmt = new BigDecimal(txtLoanableAmount.getValue().toString()); 
						...
						{
							{
								...
								panCol3Fields.setPreferredSize(new Dimension(0, 230));
								...
								{
									{
										...
										/*pnlChecks.add(Box.createHorizontalBox());
										pnlChecks.add(Box.createHorizontalBox());
										pnlChecks.add(Box.createHorizontalBox());*/
										{
											...
											{
												{
													...
													txtCOMFACpctg.getDocument().addDocumentListener(new DocumentListener() {
														...
														private void ComputeCommfac() {
															...
															System.out.println("dbLoanAmt: " + dbLoanAmt);
															System.out.println("dbCommFac: " + dbCommFac);
							...
							}
							{
								...
								pnlCol3.add(panButtons, BorderLayout.CENTER); 
								{
									{
	...
	private String sqlClients() {
		return "SELECT * FROM jsearch WHERE EXISTS(SELECT * FROM rf_buyer_status " +
			...
			"AND pbl_id = \"PBL ID\" AND seq_no = \"Sequence\" AND byrstatus_id = '48'" +
	...
	private void displayClientDetails(String entity_id, String proj_id, String pbl_id, Integer seq_no) { 
		String SQL = 
			"SELECT a.entity_id, get_client_name(a.entity_id), a.projcode, b.proj_name, " +
				"a.pbl_id, get_unit_description(a.projcode, a.pbl_id), a.seq_no,\n" + 
				"c.term, c.int_rate, c.loanable_amount, '2000'::numeric(19, 2) as proc_fee, '0.00'::numeric(19, 2) as retention_fee\n" + 
				"FROM rf_sold_unit a\n" + 
	...
	private void displayPaymentDetails(String receipt_no) {
		...
		*/
		...
		String SQL = "SELECT a.entity_id, get_client_name(a.entity_id), a.proj_id, b.proj_name, a.pbl_id, c.description, a.seq_no, a.term, a.rate, a.actual_date, a.client_seqno, a.or_no, a.acct_no,\n" + 
				...
				"COALESCE(a.com5, 0.00), COALESCE(a.rightofway, 0.00) \n" + 
				"FROM rf_pagibig_lnrel a\n" + 
				"LEFT JOIN mf_project b ON b.proj_id = a.proj_id\n" + 
				"LEFT JOIN mf_unit_info c ON c.proj_id = a.proj_id AND c.pbl_id = a.pbl_id\n" + 
				"LEFT JOIN mf_bank d ON d.bank_id = a.bank_id\n" + 
	...
	private Boolean toSave() {
		...
		if(txtReceiptNo.getText().trim().equals("")){
			JOptionPane.showMessageDialog(this.getTopLevelAncestor(), "Please input Receipt No.", "Save", JOptionPane.WARNING_MESSAGE);
			return false;
		}
		if(txtAccountNo.getText().trim().equals("")){
			JOptionPane.showMessageDialog(this.getTopLevelAncestor(), "Please input Account No.", "Save", JOptionPane.WARNING_MESSAGE);
			return false;
		}
		if(txtCheckNo.getText().trim().equals("")){
			JOptionPane.showMessageDialog(this.getTopLevelAncestor(), "Please input Check No.", "Save", JOptionPane.WARNING_MESSAGE);
			return false;
		}
		...
		if(lookupBranch.getValue() == null){
			JOptionPane.showMessageDialog(this.getTopLevelAncestor(), "Please select Branch.", "Save", JOptionPane.WARNING_MESSAGE);
			return false;
		}
	...
	private void saveTransaction() {
		...
		BigDecimal loanable_amount = (BigDecimal) txtLoanableAmount.getValue();
		...
		if (blnEdit) {
			...
			if (FncGlobal.GetInteger("select count(*)::int from rf_payments where entity_id = '"+entity_id+"' and check_no = '"+check_no+"'")==Integer.parseInt("0")) {
				...
				String SQL = "SELECT sp_tag_lnrel('"+ entity_id +"', '"+ proj_id +"', '"+ pbl_id +"', "+ seq_no +", "+ term +", "+ rate +",\n" + 
						"NULLIF('"+ actual_date +"', 'null')::TIMESTAMP, '"+ client_seqno +"', '"+ receipt_no +"', '"+ account_no +"', " +
						"'"+ check_no +"', NULLIF('"+ check_date +"', 'null')::TIMESTAMP,\n" + 
						"'"+ bank_id +"', '"+ branch_id +"', "+ loanable_amount +", ARRAY"+ less.toString() +"::NUMERIC[], " +
						"ARRAY"+ retentions +"::NUMERIC[], "+ net_proceeds +", '"+ UserInfo.EmployeeCode +"');";
