...
public class pnlSupplierAddress extends JPanel implements _GUI, ActionListener {
	...
	public pnlSupplierAddress(boolean isDoubleBuffered) {
		super(isDoubleBuffered);
		initGUI();
	}

	public pnlSupplierAddress(LayoutManager layout) {
		super(layout);
		initGUI();
	}

	public pnlSupplierAddress(LayoutManager layout, boolean isDoubleBuffered) {
							...
							try {
								if(tblAddress.getSelectedRows().length ==1){
									...
									if(rec_status.equals("I")){
										...
										si.btnState(true, true, false, false, false);
									}else{
										cmbRecStatus.setSelectedItem("Active");
										si.btnState(true, true, true, false, false);
									}
									...
									if(si.canEdit() == false){
										si.btnState(false, false, false, false, false);
									}
	...
	public void newAddress(){//NEW ADDRESS
		...
		si.setComponentsEditable(pnlUpper, true);
	...
	public void editAddress(){//EDITING OF ADDRESS
		if (tblAddress.getSelectedRows().length == 1){
			...
			tblAddress.clearSelection();
			si.btnState(true, false, false, false, false);
		}
	}
