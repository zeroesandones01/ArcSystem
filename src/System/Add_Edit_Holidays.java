...
public class Add_Edit_Holidays extends _JInternalFrame {
	...
	static String title = "Add/Edit Holidays";
	...
	private String choosedatevalue;
	...
	public Add_Edit_Holidays(){
		super(title,true,true,false,false);
		initGui();
	}
	public Add_Edit_Holidays(String title){
		super(title);
		initGui();
	}
	public Add_Edit_Holidays(String title,boolean resizable,boolean closable,boolean maximizable,boolean iconifiable){
...

		{
			...
			}
			{
				...
				}
				{
					...
					tblHoliday.hideColumns("HOLIDAY OTCODE ","DATE CREATED","REC ID","CREATED BY ");
				...
				}
				{
					...
					btnSave.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent e) {
							...
							System.out.printf("Value of Date:" + dateVal);
							System.out.println("");
	...
	//METHOD TO DISPLAY LIST OF HOLIDAYS IN THE DATABASE
	private void displayValue(DefaultTableModel model_HolidaySetterTable,JList rowHeadHoliday){
		...
		String query = "SELECT rec_id,holidays,holiday_desc,holiday_type,halfday,created_by,holiday_otcode,date_created FROM mf_holidays";
...
//			db.executeUpdate(query, true);
	...
	//METHOD TO UPDATE AN EXISTING HOLIDAY IN THE DATABASE
	...
	//METHOD THAT LISTENS WHENEVER A TABLE HAS BEEN CLICKED IN THE MODULE
	...
	private boolean DuplicateValidation(Date chooseDateValue){
		pgSelect db = new pgSelect();
		String SQL = "SELECT EXISTS (Select * from mf_holidays where holidays = '"+chooseDateValue+"')";
		db.select(SQL);

		if (db.getResult()[0][0].equals(true)) {
